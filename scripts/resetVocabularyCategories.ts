...
async function main() {
  ...
  for (let index = 0; index < vocabularies.length; index += batchSize) {
    ...
    await prisma.$transaction(
      batch.map((vocab) => {
        ...
        const resolved =
          categoryMap.has(predicted) ? predicted : classifier.defaultCategory;
    ...
    process.stdout.write(
      `  ➜ Hoàn thành ${Math.min(index + batch.length, vocabularies.length)}/${
        vocabularies.length
      } bản ghi\r`,
    );
  }
  ...
  console.table(
    Object.entries(stats).map(([name, count]) => ({ Chủ_đề: name, Số_lượng: count })),
  );
}
