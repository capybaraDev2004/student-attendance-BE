...
async function ensureNumberCategory() {
  ...
  if (!category) {
    if (!TARGET_VOCABULARY_CATEGORIES.includes(NUMBER_CATEGORY_NAME)) {
      throw new Error(
        `Chưa thêm "${NUMBER_CATEGORY_NAME}" vào TARGET_VOCABULARY_CATEGORIES`,
      );
    }
...
async function main() {
  ...
  let updated = 0;
  const reclassified: { id: number; chinese: string; meaning: string }[] = [];
  ...
  if (reclassified.length) {
    ...
    reclassified.slice(0, 20).forEach((item) => {
      const meaningShort =
        item.meaning.length > 40 ? `${item.meaning.slice(0, 40)}...` : item.meaning;
